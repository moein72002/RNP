...
def get_transform(dataset_name, input_height, input_width, train=True, random_crop_padding=4):
    ...
    # Conditional transformation to convert to PIL Image if not already one
    transforms_list.append(
        transforms.Lambda(lambda img: transforms.ToPILImage()(img) if not isinstance(img, Image.Image) else img))
